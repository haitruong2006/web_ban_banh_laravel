		...
		<div id="content">
					...
					<tbody>
						...
						<tr class="cart_item">
							<td class="product-name">
								<div class="media">
									<img class="pull-left" src="/product/{{ $product['item']['image'] }}" width="100px" alt="">
							...
							<td class="product-price">
								<span class="amount">{{ $product['item']['promotion_price']>0?number_format($product['item']['promotion_price']):number_format($product['item']['unit_price']) }}</span>
							</td>



							<td class="product-quantity">
								<input type="number" value="{{ $product['qty'] }}" style="width=10px">
							</td>

							<td class="product-subtotal">
								<span class="amount">{{ $product['item']['promotion_price']>0?number_format($product['qty'] * $product['item']['promotion_price']):number_format($product['qty']*$product['item']['unit_price']) }}</span>
							</td>
					...
					<tfoot>
						<tr>
							<td colspan="6" class="actions">
								...
								<button onclick="window.location='{{ route('dathang') }}'" type="submit" class="beta-btn primary" name="proceed">Proceed to Checkout <i class="fa fa-chevron-right"></i></button>
							</td>
						</tr>
					</tfoot>
				</table>
			...
			<div class="cart-collaterals">
				...
				<div class="cart-totals pull-right">
					...
					<div class="cart-totals-row"><span>Order Total:</span> <span>{{ number_format(Session('cart')->totalPrice) }}
                     @if(Session('value_price'))
                        <div class="cart-totals-row"><span>Shipping:</span> <span>{{ number_format(Session('value_price')) }}</span></div>
                    @endif
                    @if(Session('value_price'))
                        <div class="cart-totals-row"><span>Order Total:</span> <span>{{ number_format(session('cart')->totalPrice - Session('value_price')) }}</span></div>
                    @endif -->
                    <div class="cart-totals pull-right">
					<div class="cart-totals-row"><h5 class="cart-total-title">Cart Totals</h5></div>
                    @if(Session('cart'))
					<div class="cart-totals-row"><span>Cart Subtotal:</span> <span>{{ number_format(Session('cart')->totalPrice) }}</span></div>
                    @endif
					<div class="cart-totals-row"><span>Shipping:</span> <span>{{ Session('value_price')!=' '?number_format(Session('value_price')):' ' }}</span></div>
					<div class="cart-totals-row"><span>Order Total:</span> <span>{{ Session('value_price')!=' '?number_format(Session('cart')->totalPrice - Session('value_price')):Session('cart')->totalPrice }}</span></div>
				</div>
				</div>
